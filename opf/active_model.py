"""OPF linearizado com decisões apenas de potência ativa."""
...
def build_active_power_model(
    case: Case,
    tie_break_penalty: float = DEFAULT_TIE_BREAK_PENALTY,
) -> pyo.ConcreteModel:
    """Constrói um LinDistFlow sem potência reativa e sem perdas."""
    ...
    branch_at = {
        bus: case.branches[case.parent_branch[bus]]
        for bus in non_root
    }
    resistance = {
        bus: branch_at[bus].impedance_pu(
            base, case.buses[branch_at[bus].from_bus]
        )[0]
        for bus in non_root
    }
    tap = {bus: branch_at[bus].tap_ratio for bus in non_root}
    branch_limit = {
        bus: base.pu_power(branch_at[bus].s_max_kva)
        for bus in non_root
    }
    load = {
        bus: base.pu_power(case.buses[bus].p_load_kw.to_numpy())
    ...
    bess_at = {
        bus: [device for device in case.bess if device.bus == bus]
        for bus in case.buses
    }
    pv_at = {
        bus: [device for device in case.pv if device.bus == bus]
        for bus in case.buses
    }
    pv_available = {
        device.id: base.pu_power(device.avail_kw.to_numpy())
    ...
    m.P = pyo.Var(
        m.J,
        m.T,
        bounds=lambda _m, bus, _t: (-branch_limit[bus], branch_limit[bus]),
    )
    m.pimp = pyo.Var(
        m.T,
        domain=pyo.NonNegativeReals,
        bounds=(0.0, base.pu_power(case.grid.p_import_max_kw)),
    )
    m.pexp = pyo.Var(
        m.T,
        domain=pyo.NonNegativeReals,
        bounds=(0.0, base.pu_power(case.grid.p_export_max_kw)),
    )
    ...
    m.pch = pyo.Var(
        ...
        bounds=lambda _m, device_id, _t: (
            0.0,
            base.pu_power(min(
                bess[device_id].p_charge_max_kw,
                bess[device_id].s_max_kva,
            )),
        ),
    )
    m.pdis = pyo.Var(
        ...
        bounds=lambda _m, device_id, _t: (
            0.0,
            base.pu_power(min(
                bess[device_id].p_discharge_max_kw,
                bess[device_id].s_max_kva,
            )),
        ),
    )
    m.soc = pyo.Var(
        ...
        m.T,
        bounds=lambda _m, device_id, _t: (
            bess[device_id].soc_min_frac
            * base.pu_energy(bess[device_id].e_cap_kwh),
            bess[device_id].soc_max_frac
            * base.pu_energy(bess[device_id].e_cap_kwh),
        ),
    )
    ...
    def pv_bounds(_m, device_id, t):
        device = pv[device_id]
        upper = min(
            pv_available[device_id][t],
            base.pu_power(device.p_max_kw),
            base.pu_power(device.s_max_kva),
        )
    ...
    def injection(_m, bus, t):
        value = -load[bus][t]
        value += sum(
            _m.pdis[device.id, t] - _m.pch[device.id, t]
            for device in bess_at[bus]
        )
    ...
    def active_balance(_m, bus, t):
        outgoing = sum(
            _m.P[child, t]
            for child in case.children.get(bus, [])
        )
    ...
    def voltage_drop(_m, bus, t):
        ...
        return _m.v[bus, t] == tap[bus] ** 2 * (
            _m.v[parent, t] - 2.0 * resistance[bus] * _m.P[bus, t]
        )
    ...
    def soc_balance(_m, device_id, t):
        ...
        delta = (
            device.eta_charge * _m.pch[device_id, t]
            - _m.pdis[device_id, t] / device.eta_discharge
        ) * dt
        initial = (
            device.soc_init_frac * base.pu_energy(device.e_cap_kwh)
        )
    ...
    def terminal_soc(_m, device_id):
        device = bess[device_id]
        if not device.cyclic_soc:
            return pyo.Constraint.Skip
        return _m.soc[device_id, periods[-1]] == (
            device.soc_init_frac * base.pu_energy(device.e_cap_kwh)
        )
    ...
    m.energy_cost = pyo.Expression(
        expr=base.s_base_kva * dt * sum(
            price[t] * (m.pimp[t] - feed_in_ratio * m.pexp[t])
            for t in periods
        )
    )
    m.tie_break_cost = pyo.Expression(
        expr=tie_break_penalty * base.s_base_kva * dt * (
            ...
            + sum(
                m.pch[device_id, t] + m.pdis[device_id, t]
                for device_id in m.S for t in periods
            )
        )
    )
    m.cost = pyo.Objective(
        expr=m.energy_cost + m.tie_break_cost,
        sense=pyo.minimize,
    )
    return m
...
def solve_active_power_opf(
    ...
    """Resolve o OPF ativo linearizado e anexa os resultados ao caso."""
...
def _attach_active_results(model, case: Case, status: str) -> Case:
    ...
    for bus_id in (bus for bus in case.buses if bus != case.root):
        ...
        branch.result = BranchResult(
            p_kw=series([
                base.to_kw(value(model.P[bus_id, t]))
                for t in periods
            ]),
            q_kvar=zeros.copy(),
            loss_kw=zeros.copy(),
    ...
    for device in case.bess:
        charge = series([
            base.to_kw(value(model.pch[device.id, t]))
            for t in periods
        ])
        discharge = series([
            base.to_kw(value(model.pdis[device.id, t]))
            for t in periods
        ])
        soc = series([
            base.to_kwh(value(model.soc[device.id, t]))
            for t in periods
        ])
    ...
    for device in case.pv:
        generation = series([
            base.to_kw(value(model.ppv[device.id, t]))
            for t in periods
        ])
    ...
    voltages = [
        float(bus.result.v_pu.min())
        for bus in case.buses.values()
    ]
    voltage_maxima = [
        float(bus.result.v_pu.max())
        for bus in case.buses.values()
    ]
    case.summary = Summary(
        ...
        pv_generated_kwh=sum(
            float(device.result.gen_kw.sum()) for device in case.pv
        ) * case.dt_h,
        pv_curtailed_kwh=sum(
            float(device.result.curtail_kw.sum()) for device in case.pv
        ) * case.dt_h,
        losses_kwh=0.0,
